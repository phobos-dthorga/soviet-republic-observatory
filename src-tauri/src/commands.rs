...
pub fn get_setup_state(state: State<'_, AppState>) -> Result<SetupState, CommandError> {
    setup_state(&state.repository).map_err(Into::into)
}
...
pub fn get_latest_receiver_dataset(
    ...
    state.repository.load_latest_dataset().map_err(Into::into)
}
...
pub fn configure_directory(
    ...
    match kind {
        DirectoryKind::Save => {
            ...
            state
                .repository
                .set_setting(SAVE_DIRECTORY_KEY, &canonical)?;
        }
        DirectoryKind::Game => {
            let media = resolve_game_media_directory(&selected)?;
            state
                .repository
                .set_setting(GAME_MEDIA_DIRECTORY_KEY, &media)?;
        }
    }
    setup_state(&state.repository).map_err(Into::into)
}
...
pub fn observe_latest_save(
    ...
    let inserted = state.repository.save_inspection(&inspection)?;
    let dataset = state.repository.load_dataset(&inspection.payload_hash)?;
...
fn setup_state(repository: &ObservationRepository) -> Result<SetupState, ObservatoryError> {
    let save_path = repository.get_setting(SAVE_DIRECTORY_KEY)?;
    let game_path = repository.get_setting(GAME_MEDIA_DIRECTORY_KEY)?;
    ...
    Ok(SetupState {
        ...
        game_directory,
        save_candidates,
        observed_saves: repository.observation_count()?,
        game_vocabularies,
    })
}
