ALTER TABLE warehouse_projection_jobs RENAME TO warehouse_projection_jobs_v23;
...
CREATE TABLE warehouse_projection_jobs (
    ...
    projection_kind TEXT NOT NULL CHECK (
        projection_kind IN (
            ...
            'environment_observation', 'resource_registry_snapshot', 'overlay_state',
            'branch_membership', 'rebuild'
        )
    ),
...
SELECT projection_id, projection_kind, source_identity, status, requested_at_ms,
       started_at_ms, applied_at_ms, attempt_count, error_code
FROM warehouse_projection_jobs_v23;

DROP TABLE warehouse_projection_jobs_v23;
...
SELECT 'environment:' || interpretation_id, 'environment_observation', interpretation_id,
       'pending', indexed_at_ms
FROM environment_interpretation_variants;
